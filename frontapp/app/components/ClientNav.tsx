...
export default function ClientNav() {
  ...
  const disabled =
    submitting || !me?.email || !form.title.trim() || !form.content.trim();

  // 모달 열릴 때 사용자 정보가 없으면 한번만 로드 (경로는 필요에 맞게 수정)
  useEffect(() => {
    if (!open || me) return;
    (async () => {
      try {
        setMeError(null);
        // 필요에 맞게 /api/admin/v1/me 또는 /api/user/v1/me 등으로 수정
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const msg =
        ...
        "문의 접수 중 오류가 발생했습니다.";
      setDoneMsg(msg);
      // 모달 닫지 않고 메시지만 표시하고 싶으면 setOpen(false) 제거 유지
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <>
      ...
      >
        <form onSubmit={onSubmit} className="space-y-3" noValidate>
          ...
          {doneMsg && (
            <div className="rounded-xl border p-3 text-xs text-slate-700 bg-slate-50">
              {doneMsg}
            </div>
          )}
