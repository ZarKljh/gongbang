...
export default function DeliveryPage() {
    ...
    const getStatusBarIndex = (deliveryStatus: string | undefined) => {
        if (!deliveryStatus) return 0
        const s = deliveryStatus.toUpperCase()

        if (s.includes('완료') || s.includes('DELIVERED')) return 2
        if (s.includes('출발') || s.includes('배송중') || s.includes('IN_TRANSIT')) return 1
    ...
    const statusIndex = getStatusBarIndex(data.deliveryStatus)
                ...
                <section className={styles.card}>
                    <div className={styles.statusHeader}>
                        <span className={styles.statusDate}>
                            {formatDateShort(data.steps[0]?.time ?? data.orderCreatedDate)}
                        </span>
